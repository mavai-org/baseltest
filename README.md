...
Frequently reached-for flags: `--html-report <path>` on `test` renders the report inline as part of the run (the same renderer as `basel report test`, so the two outputs are identical); `--baseline-dir`, `--verdict-dir`, and `--explorations-dir` relocate the artefact directories. Everything a run generates lands under `_baseltest/`.
