...
Three further keys tune how the model client behaves, and — like `model` and `temperature` — they are factors of the configuration, not provider switches: declare them and the provider that serves the run maps them onto its wire format. `top-p: <number in (0, 1]>` is the other common sampling knob (nucleus sampling), passed through wherever the provider has a slot for it, exactly like `temperature`. `thinking: adaptive` lets the model choose its own deliberation depth per response (default `none` — today's behaviour); it changes the response distribution, so a baseline measured under one setting refuses a test under the other, like any drifted configuration key. `prompt-caching: true` asks the provider to cache the compiled system prompt (default `false`); caching reuses computation over an identical prefix, so correctness is untouched and the effect is confined to latency and cost. Provider support for `thinking` and `prompt-caching` follows the `response-schema` rule above: `measure` and `test` refuse up front on a provider that cannot honour an active key, while an `explore` grid drops it for that configuration with a console note. (Currently the `anthropic` provider realises both; on that provider, `thinking: adaptive` cannot be combined with an explicit `temperature:` or `top-p:` — the API constrains sampling parameters while thinking, and baseltest refuses the combination at load time.) One latency note for `prompt-caching`: the first, cache-writing invocation simply lands as the slowest recorded point, and if the provider's cache entry expires mid-run the population mixes cached and uncached samples — the empirical percentiles absorb this descriptively, so a bimodal p99 under caching is the cache's signature, not service degradation.

When the measured endpoint is an **LLM gateway** (litellm, and the same shape in other routers) rather than a vendor API, one key needs declaring that the others do not. Capability is a property of the gateway *and* the upstream model its alias currently fronts — not of the adapter, which sees only an OpenAI-compatible surface and a mutable alias, and so cannot tell from the protocol whether that alias honours structured output, prompt caching or thinking. On `provider: litellm` you therefore state it: `capabilities: [response-schema, prompt-caching, thinking]` lists what the endpoint honours, and only a listed key is sent. An undeclared key behaves exactly as an unsupported vendor key — refused under `measure`/`test`, dropped-with-note under `explore` — and a declared key the adapter has no wire form for is refused at load (as are `prompt-caching` and `thinking` on the plain omitted-provider path, whose generic body cannot encode them; `response-schema` there is already on). The allowance is part of the service's identity like every other key: widening what a service may send changes what was measured, so a later `test` refuses against a baseline taken without it. One hazard baseltest deliberately does not police: a gateway operator repointing an alias to a different upstream leaves the recorded model id byte-identical though the service has changed — an operational matter the operator owns, and one you can guard by pinning the real upstream identity as a covariate.

`max-tokens: <1…16000>` is the ceiling on how many tokens the model may return — and it shapes the response as strongly as any other parameter: it is the difference between a complete answer and one truncated mid-sentence. For that reason it is not optional in the way a default-able convenience is: an *unstated* ceiling still resolves to a recorded value (`4096`) that is fingerprinted into the service's identity like every other key, so a measured service is never left with a silent, invisible cap. State it explicitly when the default is wrong for your outputs — long structured responses routinely need more. Each provider maps it to its own wire parameter (`max_tokens` for Anthropic and Mistral, `max_completion_tokens` for OpenAI, `num_predict` for Ollama); a provider grid therefore holds the ceiling *constant* across arms rather than letting each provider's own default vary it underneath the comparison. Two limits: ceilings above `16000` need streaming, which baseltest does not yet do, so they are refused at load; and under `thinking: adaptive` the ceiling bounds thinking *and* answer together, so a ceiling at or below the model's thinking floor is refused up front — the reasoning would consume the whole budget and leave nothing for the answer.
...
Views are not limited to the stock parsers (`json`, `xml`, `yaml`). A transformation registered in code (`@transform` in `mavai-bindings.py` — the [basket-builder example](../examples/language-model/mavai-bindings.py) registers a small in-process judge) produces a view like any other, and when it returns *structure* — a dict or list, or a parsed XML element — `path:` addresses it the same way. The path's own syntax picks the language: an expression starting with `$` is JSONPath (RFC 9535), anything else is XPath 1.0, so a transformation returning a parsed XML element takes XPath conditions (one returning XML as *text* does not — return the parsed element). Code gives no load-time guarantee of what a transformation returns, so the value's type is checked on every trial: a `$`-rooted path over a view that turns out to hold plain text fails that trial with the type named, exactly as an empty selection does. The whole view taxonomy in one rule: a view holding **text** takes the string forms (`equals`, `contains`, `matches`, `one-of`); a view holding **structure** takes `path:` (whose selected scalars are judged by the string form it qualifies); a string form applied directly to structure is a per-trial type failure, never a silent stringification; and `parses: <view>` makes computing the view itself the check.
...
Every run persists its artefacts, so reporting never requires re-execution — and rendering is not `basel`'s job. HTML reports are the work of the **`mavai`** command, which renders the interchange artefacts baseltest persists (verdict records, baselines, `mavai-explore-1` and `mavai-optimize-1` documents). The tool is developed in the [mavai-report](https://github.com/mavai-org/mavai-report) project; obtain it from there, and see that project's `README.md` for installation instructions.

```bash
mavai verdict _baseltest -o report.html              # test-run verdicts and standings
mavai measure _baseltest -o report.html              # measurement (baseline) records
mavai explore _baseltest/explorations -o report.html # exploration comparison
mavai optimize _baseltest/optimizations -o report.html # optimization comparison
```

Each report is one self-contained page (embedded CSS, no JavaScript, no external assets) that opens offline in any browser; it goes to stdout unless `-o FILE` is given, and diagnostics go to stderr. Prefer one step? `--html-report <path>` on `test`, `measure`, `explore` or `optimize` hands that run's artefacts to the same tool as part of the run — the report is identical either way, because it is the same renderer over the same artefacts. Did not ask for a report at the time? `basel report <kind> [-o page.html]` draws it from what the run already wrote — `basel report explore`, `basel report test`, and so on — without invoking the service again. It produces the same page the flag would have, because it is the same renderer over the same artefacts; with no `-o` it goes to stdout. For `explore` and `optimize` you may name a contract to narrow the report to that contract's runs.
