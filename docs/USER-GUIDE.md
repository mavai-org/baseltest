...
**Signalling an unusable response vs. a defect.** The two channels are distinct on purpose (the family's expected-failure-vs-defect line): a `TransformError` is an *anticipated* bad response — a failed trial that counts toward the denominator, exactly like a false postcondition. *Any other* exception escaping the transform is a **defect** — a bug in the testing machinery, never a countable outcome and never a sample. A defect no longer unwinds the whole run: it stops *its* configuration with an actionable diagnosis (naming the transform, the criterion and postcondition under evaluation, the exception, and a bounded excerpt of the driving input), and every other configuration in an `explore`, `measure`, or `optimize` grid runs to completion and writes its artefacts. A partial run is a reported outcome — the summary names the configuration a defect stopped, and the exit status is non-zero.

**Catch broadly when you parse.** An author who can foresee a bad response should turn it into a `TransformError` rather than let it escape — and should catch **broadly** (`except ValueError`), not narrowly (`except json.JSONDecodeError`). A stochastic service emits degenerate draws, and `json.loads` raises a plain `ValueError` — not always a `JSONDecodeError` — on some of them (for instance an enormous but syntactically valid integer the platform will not realise). A narrow catch lets that through as a spurious defect; a broad one records it as the transform/no-value failed trial it is. The stock `json`, `xml`, and `yaml` transforms already catch broadly, so the built-in views are never abortable.
